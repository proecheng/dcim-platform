<template>
  <div class="device-detail-page">
    ...
    >
      <template #extra>
        <el-button type="primary" @click="router.push('/collection/device-manage')">返回设备管理</el-button>
...
async function loadDetail() {
  ...
  if (!Number.isInteger(id) || id <= 0) {
    ...
    loadError.value = {
      title: '设备 ID 无效',
      description: '当前地址中的设备 ID 不合法，请返回设备管理重新选择设备。'
    }
    stopRefreshTimer()
    return
  }
  ...
  try {
    const res = await getDeviceDetail(id, { silentError: true })
    ...
    loadError.value = status === 404
      ? {
          title: '未找到设备',
          description: `设备 ${id} 不存在，可能已被删除或您当前无权查看。`
        }
      : {
          title: '设备详情加载失败',
          description: '暂时无法获取设备详情，请稍后重试。'
        }
    stopRefreshTimer()
  } finally {
    loading.value = false

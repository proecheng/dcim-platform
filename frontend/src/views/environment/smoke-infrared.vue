<template>
  <div class="smoke-infrared-monitor">
    <!-- 顶部统计卡片 -->
          ...
          <div class="zone-body">
            <div class="zone-metric-row">
              <div class="zone-metric">
                <span class="metric-label">烟雾</span>
                <span class="metric-value">{{ zone.smokeCount }}</span>
              </div>
              <div class="zone-metric">
                <span class="metric-label">烟雾告警</span>
                <span class="metric-value alarm-val">{{ zone.smokeAlarmCount }}</span>
              </div>
            </div>
            <div class="zone-metric-row">
              <div class="zone-metric">
                <span class="metric-label">红外</span>
                <span class="metric-value">{{ zone.irCount }}</span>
              </div>
              <div class="zone-metric">
                <span class="metric-label">红外告警</span>
                <span class="metric-value alarm-val">{{ zone.irAlarmCount }}</span>
              </div>
            </div>
    ...
    <el-drawer
      ...
      :title="selectedZone ? `${selectedZone.areaCode} — 烟雾/红外传感器` : '烟雾/红外传感器'"
      size="560px"
      direction="rtl"
    >
        ...
        >
          <div class="sensor-header">
            ...
            <div class="sensor-tags">
              <el-tag :type="sensorTypeTagType(sensor.device_type)" size="small" effect="plain">
                {{ sensorTypeText(sensor.device_type) }}
              </el-tag>
              <el-tag :type="statusTagType(sensor.status)" size="small">
                {{ statusText(sensor.status, sensor.device_type) }}
              </el-tag>
            </div>
          </div>
          <div class="sensor-value">
            <span>{{ sensor.value_text || statusText(sensor.status, sensor.device_type) }}</span>
          </div>
        </div>
      </div>
        ...
        <div class="sensor-detail">
          <div class="detail-info">
            ...
            <div class="detail-row">
              ...
              <el-tag :type="sensorTypeTagType(selectedSensor.device_type)" size="small" effect="plain">
                {{ sensorTypeText(selectedSensor.device_type) }}
              </el-tag>
            </div>
            <div class="detail-row">
              <span class="detail-label">当前状态</span>
              <el-tag :type="statusTagType(selectedSensor.status)" size="small">
                {{ statusText(selectedSensor.status, selectedSensor.device_type) }}
              </el-tag>
            </div>
            <div class="detail-row">
          ...
          <!-- 消防联动策略状态（仅烟雾传感器告警时显示） -->
          <div v-if="selectedSensor.device_type === 'SMOKE' && selectedSensor.status === 'alarm'" class="linkage-section">
            <div class="linkage-title">消防联动策略</div>
            <div v-if="matchedFirePolicies.length > 0" class="linkage-list">
              <div v-for="policy in matchedFirePolicies" :key="policy.id" class="linkage-item">
                <el-icon :size="14" class="linkage-icon"><Connection /></el-icon>
                <span class="linkage-name">{{ policy.name }}</span>
                <el-tag :type="policy.is_enabled ? 'success' : 'info'" size="small">
                  {{ policy.is_enabled ? '已启用' : '已禁用' }}
                </el-tag>
              </div>
            </div>
            <div v-else class="linkage-empty">
    ...
    <el-card shadow="hover" class="table-card">
      <template #header>
        <div class="card-header">
          <div class="header-left">
            ...
            <span>烟雾/红外传感器数据</span>
          </div>
          <div class="header-filters">
            <el-select v-model="filterType" placeholder="全部类型" clearable size="default" style="width: 120px;">
      ...
      <el-table :data="filteredTableData" stripe height="420" v-loading="loading" :row-class-name="tableRowClass">
        ...
        <el-table-column label="类型" width="80">
          <template #default="{ row }">
            <el-tag :type="sensorTypeTagType(row.device_type)" size="small" effect="plain">
              {{ sensorTypeText(row.device_type) }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="当前状态" width="120">
          <template #default="{ row }">
            <el-tag :type="statusTagType(row.status)" size="small">{{ statusText(row.status, row.device_type) }}</el-tag>
          </template>
        </el-table-column>
...
import { Warning, Search, Monitor, WarningFilled, Bell, Connection } from '@element-plus/icons-vue'
import { getAlarmList, type AlarmInfo } from '@/api/modules/alarm'
import { useSmokeInfraredData, type SmokeIRZoneGroup } from '@/composables/useSmokeInfraredData'
...
const {
  ...
  firePolicies,
  fetchData,
} = useSmokeInfraredData()

// ── 统计卡片配置（5 个） ──
const statCards = computed(() => [
  {
    label: '烟雾传感器',
    value: smokeTotal.value,
    icon: Monitor,
...
const selectedZone = ref<SmokeIRZoneGroup | null>(null)
const selectedSensor = ref<RealtimeData | null>(null)
const sensorAlarms = ref<AlarmInfo[]>([])
const alarmLoading = ref(false)

function handleZoneClick(zone: SmokeIRZoneGroup) {
...
const filteredTableData = computed(() => {
  ...
  if (filterType.value) {
    data = data.filter(d => d.device_type === filterType.value)
  }
...
function statusText(status: string, deviceType?: string): string {
  if (status === 'alarm') {
    return deviceType === 'SMOKE' ? '烟雾告警' : '入侵告警'
  }

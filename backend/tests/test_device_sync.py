...
class TestUPSCircuitInference:
    ...
    def test_ups_f2_format2(self, sync_service, circuit_map):
        """测试 F2-UPS-XX 格式"""
        device = MagicMock(spec=Device)
    ...
    def test_ups_unknown_floor(self, sync_service, circuit_map):
        """测试未知楼层 UPS"""
        device = MagicMock(spec=Device)
...
class TestPDUCircuitInference:
    ...
    def test_pdu_f2_priority(self, sync_service, circuit_map):
        """测试 PDU-F2-XX 优先级"""
        result = sync_service._infer_pdu_circuit("PDU-F2-01", None, circuit_map)
        assert result == 10

    def test_pdu_f3_priority(self, sync_service, circuit_map):
        """测试 PDU-F3-XX 优先级"""
        result = sync_service._infer_pdu_circuit("PDU-F3-02", None, circuit_map)
        assert result == 11

    def test_pdu_area_a1_by_code(self, sync_service, circuit_map):
        """测试通过编码识别 A1 区域"""
        result = sync_service._infer_pdu_circuit("PDU-A-01", None, circuit_map)
        assert result == 13

    def test_pdu_area_b1_by_area_code(self, sync_service, circuit_map):
        """测试通过 area_code 识别 B1 区域"""
        result = sync_service._infer_pdu_circuit("PDU-01", "B1", circuit_map)
        assert result == 14

    def test_pdu_unknown(self, sync_service, circuit_map):
        """测试未知 PDU"""
        result = sync_service._infer_pdu_circuit("PDU-UNKNOWN", None, circuit_map)
        assert result is None
...
class TestHVACCircuitInference:
    ...
    def test_ca_area_specific_priority(self, sync_service, circuit_map):
        """测试区域冷通道优先级次之 (CA-A01 → C-CA-A-01)"""
        result = sync_service._infer_hvac_circuit("CA-A01", circuit_map)
        assert result == 23, "CA-A01 应绑定到 C-CA-A-01 (区域特定)"

    def test_ca_generic_priority(self, sync_service, circuit_map):
        """测试通用冷通道优先级最低 (CA-XX → C-CA-GENERIC)"""
        result = sync_service._infer_hvac_circuit("CA-99", circuit_map)
    ...
    # 水泵测试
    def test_pmp_chilled_water_pump(self, sync_service, circuit_map):
        """测试冷冻水泵 (PMP-F1-01~04 → C-CHWP-01)"""
        for i in [1, 2, 3, 4]:
            result = sync_service._infer_hvac_circuit(f"PMP-F1-0{i}", circuit_map)
            assert result == 30, f"PMP-F1-0{i} 应绑定到 C-CHWP-01"

    def test_pmp_cooling_water_pump(self, sync_service, circuit_map):
    ...
    def test_pmp_round_robin_even(self, sync_service, circuit_map):
        """测试水泵轮询分配 - 偶数 (PMP-F1-06 → C-PMP-02)"""
        result = sync_service._infer_hvac_circuit("PMP-F1-06", circuit_map)
        assert result == 33, "PMP-F1-06 应轮询到 C-PMP-02"

    def test_pmp_invalid_number_fallback(self, sync_service, circuit_map):
        """测试水泵编号无效时回退到通用回路"""
        result = sync_service._infer_hvac_circuit("PMP-F1-XX", circuit_map)
        assert result == 34, "PMP-F1-XX 应回退到 C-PMP-GENERIC"

    # 室外机测试
    def test_ac_out_priority(self, sync_service, circuit_map):
        """测试室外机绑定 (AC-OUT-XX → C-AC-OUT-01)"""
        result = sync_service._infer_hvac_circuit("AC-OUT-01", circuit_map)
    ...
    # 冷机和冷却塔
    def test_chiller(self, sync_service, circuit_map):
        """测试冷机 (CH-XX → C-CH-01)"""
        result = sync_service._infer_hvac_circuit("CH-01", circuit_map)
        assert result == 41

    def test_cooling_tower(self, sync_service, circuit_map):
        """测试冷却塔 (CT-XX → C-CT-01)"""
        result = sync_service._infer_hvac_circuit("CT-01", circuit_map)
        assert result == 42

    # 楼层空调
    def test_floor_ac(self, sync_service, circuit_map):
        """测试楼层空调 (F2-AC-XX → C-F2-AC-01)"""
        result = sync_service._infer_hvac_circuit("F2-AC-01", circuit_map)
        assert result == 44

    # 精密空调
    def test_precision_ac_a(self, sync_service, circuit_map):
        """测试精密空调 A (AC-A → C-AC-01)"""
        result = sync_service._infer_hvac_circuit("AC-A", circuit_map)
        assert result == 45

    def test_precision_ac_b(self, sync_service, circuit_map):
        """测试精密空调 B (AC-B → C-AC-02)"""
        result = sync_service._infer_hvac_circuit("AC-B", circuit_map)
        assert result == 46
...
class TestPriorityConflictResolution:
    ...
    def test_ca_f2_not_generic(self, sync_service, circuit_map):
        """
    ...
    def test_ca_f3_not_generic(self, sync_service, circuit_map):
        """测试 CA-F3-XX 优先级"""
        result = sync_service._infer_hvac_circuit("CA-F3-02", circuit_map)
        assert result == 21, "CA-F3-02 必须绑定到 C-F3-CA-01"
        assert result != 24

    def test_ca_a01_not_generic(self, sync_service, circuit_map):
        """测试 CA-A01 优先级"""
        result = sync_service._infer_hvac_circuit("CA-A01", circuit_map)
    ...
    def test_pmp_round_robin_not_generic(self, sync_service, circuit_map):
        """测试水泵轮询优先于通用回路"""
        result = sync_service._infer_hvac_circuit("PMP-F1-05", circuit_map)

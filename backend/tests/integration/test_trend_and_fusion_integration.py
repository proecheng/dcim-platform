...
class TestTrendAnalysisIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_trend_analysis_flow(self, async_session: AsyncSession):
        """测试完整的趋势分析流程"""
        # 1. 创建测试点位
        point = Point(
            code="T-TEST-01",
        ...
        await async_session.refresh(point)

        # 2. 创建趋势分析服务
        trend_service = TrendAnalysisService(async_session)
        trend_service.redis = None  # 跳过 Redis 检查
    ...
    @pytest.mark.asyncio
    async def test_trend_warning_persistence(self, async_session: AsyncSession):
        """测试趋势预警持久化"""
        # 1. 创建测试点位
        point = Point(
            code="T-TEST-02",
        ...
        async_session.add(warning)
        await async_session.commit()

        # 3. 查询验证
        result = await async_session.execute(
            select(TrendWarning).where(TrendWarning.point_id == point.id)
        )
    ...
    @pytest.mark.asyncio
    async def test_trend_warning_acknowledgement(self, async_session: AsyncSession):
        """测试趋势预警确认"""
        # 1. 创建测试点位和预警
        point = Point(
            code="T-TEST-03",
        ...
        async_session.add(warning)
        await async_session.commit()
        await async_session.refresh(warning)
        ...
        # 3. 验证确认状态
        result = await async_session.execute(
            select(TrendWarning).where(TrendWarning.id == warning.id)
        )
...
class TestSensorFusionIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_sensor_fusion_flow(self, async_session: AsyncSession):
        """测试完整的多传感器融合流程"""
        # 1. 创建融合服务
        fusion_service = SensorFusionService(async_session)
    ...
    @pytest.mark.asyncio
    async def test_fusion_record_persistence(self, async_session: AsyncSession):
        """测试融合记录持久化"""
        ...
        # 4. 查询验证
        result = await async_session.execute(
            select(SensorFusionRecord).where(SensorFusionRecord.zone_id == 1)
        )
...
class TestDiagnosisEngineIntegration:
    ...
    @pytest.mark.asyncio
    async def test_fault_tree_integration(self, async_session: AsyncSession):
        """测试故障树集成多传感器融合和趋势预警"""
    ...
    @pytest.mark.asyncio
    async def test_evidence_collection_with_fusion(self, async_session: AsyncSession):
        """测试证据收集集成融合数据"""
        ...
        # 1. 获取服务实例
        fusion_service = get_sensor_fusion_service(async_session)
        trend_service = get_trend_analysis_service(async_session)
        ...
        pressure_check = await fusion_service.check_differential_pressure(1)
...
class TestScheduledTaskIntegration:
    ...
    @pytest.mark.asyncio
    async def test_trend_analysis_task_execution(self, async_session: AsyncSession):
        """测试趋势分析定时任务执行"""
        # 模拟定时任务执行流程

...
from app.utils.deterministic import (
...
class TestSchemaDataSource:
    ...
    def test_energy_stat_data_source_optional(self):
        ...
        stat = EnergyStat(
            total_energy=1000, peak_energy=400, normal_energy=350,
            valley_energy=250, total_cost=800, peak_cost=480,
            normal_cost=280, valley_cost=100, avg_power=42, max_power=50
        )
    ...
    def test_energy_trend_data_source(self):
        ...
        trend = EnergyTrend(
            granularity="daily", data=[], total_energy=0, total_cost=0
        )
    ...
    def test_energy_comparison_data_source(self):
        ...
        comp = EnergyComparison(
            current_period=stat, previous_period=stat,
...
class TestPricingServiceIntegration:
    ...
    async def test_get_all_prices_returns_dict(self, db_session):
        ...
        pricing = ElectricityPricing(
            pricing_name="高峰电价", period_type="peak",
...
class TestEnergyDailyQuery:
    ...
    async def test_query_daily_with_data(self, db_session):
        ...
        device = PowerDevice(
            device_code="DEV-D01", device_name="测试设备1",
            device_type="IT", rated_power=100.0, is_enabled=True
        )
        ...
        result = await db_session.execute(
            select(EnergyDaily).where(
                EnergyDaily.stat_date == date(2026, 1, 15)
            )
        )
    ...
    async def test_query_daily_empty(self, db_session):
        ...
        result = await db_session.execute(
            select(EnergyDaily).where(
                EnergyDaily.stat_date == date(2099, 12, 31)
            )
        )
        records = result.scalars().all()
...
class TestEnergyMonthlyQuery:
    ...
    async def test_query_monthly_with_data(self, db_session):
        ...
        device = PowerDevice(
            device_code="DEV-M01", device_name="测试设备M1",
            device_type="IT", rated_power=100.0, is_enabled=True
        )
        db_session.add(device)
        await db_session.flush()

...
class TestFindPlansNeedingTracking:
    ...
    async def test_find_plans_needing_tracking(self, db_session):
        ...
        plan = _create_plan(db_session, opp.id, completed_at=datetime.now() - timedelta(days=10))
        await db_session.flush()
...
class TestEnergyComparisonEffect:
    ...
    async def test_calculate_energy_comparison_effect(self, db_session):
        ...
        for i in range(7):
            ...
            db_session.add(EnergyDaily(device_id=1, stat_date=d, total_energy=100, energy_cost=60))

        # 执行后数据 (7天, 每天80kWh, 48元)
        for i in range(7):
            d = tracking_start + timedelta(days=i)
            db_session.add(EnergyDaily(device_id=1, stat_date=d, total_energy=80, energy_cost=48))
...
class TestLoadShiftEffect:
    ...
    async def test_calculate_load_shift_effect(self, db_session):
        ...
        effect = await tracker._calculate_load_shift_effect(
            analysis_data, tracking_days=7, tracking_start=tracking_start, tracking_end=tracking_end
        )
...
class TestExtractDeviceIds:
    ...
    async def test_extract_device_ids(self, db_session):
        ...
        t1 = ExecutionTask(
            ...
            task_name="t1",
            parameters={"selected_devices": [1, 2, 3]},
            status="pending",
        )
        ...
        t2 = ExecutionTask(
            ...
            task_type="load_shift",
            task_name="t2",
            parameters={"selected_devices": [{"device_id": 4}]},
            status="pending",
        )
        ...
        t3 = ExecutionTask(
            ...
            task_type="load_shift",
            task_name="t3",
            target_object="device:5",
            parameters={},
            status="pending",
        )
        # 任务4: parameters.device_id
        t4 = ExecutionTask(
            plan_id=plan.id, task_type="load_shift", task_name="t4", parameters={"device_id": 6}, status="pending"
        )
...
class TestAchievementRateClamp:
    ...
    async def test_achievement_rate_clamp(self, db_session):
        ...
        # 查询保存的结果
        res = await db_session.execute(select(ExecutionResult).where(ExecutionResult.plan_id == plan.id))

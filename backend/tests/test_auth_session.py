...
async def test_user(db_session):
    """创建测试用户"""
    user = User(username="sessionuser", password_hash=get_password_hash("Test@1234"), role="admin", is_active=True)
    db_session.add(user)
...
async def test_login_creates_session(client, test_user, db_session):
    """登录成功后创建会话记录"""
    resp = await client.post(LOGIN_URL, data={"username": "sessionuser", "password": "Test@1234"})
    assert resp.status_code == 200
    data = resp.json()
    assert "access_token" in data

    # 检查 session 记录
    result = await db_session.execute(select(UserSession).where(UserSession.user_id == test_user.id))
...
async def test_concurrent_session_limit(client, test_user, db_session):
    ...
    result = await db_session.execute(
        select(UserSession).where(UserSession.user_id == test_user.id, UserSession.is_active == True)
    )
...
async def test_kicked_session_returns_401(client, test_user, db_session):
    ...
    # 用第1个 token 访问 /me
    resp = await client.get("/api/v1/auth/me", headers={"Authorization": f"Bearer {tokens[0]}"})
...
async def test_latest_session_still_works(client, test_user):
    ...
    # 最后一个 token 应该有效
    resp = await client.get("/api/v1/auth/me", headers={"Authorization": f"Bearer {tokens[-1]}"})
    assert resp.status_code == 200
...
async def test_rate_limit(client, test_user):
    ...
    # 快速发送6次错误登录
    for i in range(6):
        resp = await client.post(LOGIN_URL, data={"username": "sessionuser", "password": "wrong_password"})

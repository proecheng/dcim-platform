...
async def seed_cooling(db_session):
    ...
    cc = CoolingCapacity(
        name="A区制冷",
        location="A区/1F/Room1",
        total_cooling_kw=100.0,
        used_cooling_kw=30.0,
    )
    db_session.add(cc)
    await db_session.commit()
    return cc
...
class TestOverrideCabinet:
    ...
    async def test_override_cabinet_feasible(self, client: AsyncClient, seed_plan, seed_assets):
        ...
        resp = await client.put(
            "/api/v1/capacity/plans/1/override-cabinet",
            json={
                "target_cabinet_id": 3,
            },
        )
    ...
    async def test_override_plan_not_found(self, client: AsyncClient, seed_cabinets):
        resp = await client.put(
            "/api/v1/capacity/plans/9999/override-cabinet",
            json={
                "target_cabinet_id": 1,
            },
        )

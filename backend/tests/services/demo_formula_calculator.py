...
def demo_formula_calculator():
    ...
    print("1. 负荷率计算")
    print("-" * 40)
    result = calc.calc_load_factor(avg_load=Decimal("14000"), max_demand=Decimal("22000"))
    print("   平均负荷: 14,000 kW")
    print("   最大需量: 22,000 kW")
    print(f"   负荷率: {result}%")
    print()
    ...
    print("   可转移功率: 1,400 kW")
    print("   转移时长: 3 小时/天")
    print("   尖峰电价: 1.1 元/kWh")
    print("   低谷电价: 0.111 元/kWh")
    ...
    methods = [
        ...
        ("A2 需量控制", ["calc_demand_control_data() - 需量控制数据"]),
        (
            "A3 设备优化",
            ["calc_equipment_load_rate() - 设备负荷率", "calc_equipment_optimization_potential() - 设备优化潜力"],
        ),
        ("A4 VPP 需求响应", ["calc_vpp_response_potential() - VPP 响应潜力"]),
        ("A5 负荷调度", ["calc_load_curve_analysis() - 负荷曲线分析"]),
        ("B1 设备改造", ["calc_equipment_efficiency_benchmark() - 设备能效对标"]),
        ("辅助方法", ["_get_electricity_price() - 获取电价", "_get_working_days_in_period() - 工作日计算 ✓ 已测试"]),
    ]

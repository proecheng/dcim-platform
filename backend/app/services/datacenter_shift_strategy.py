...
from ..models.topology_config import CoolingZoneUnit, CoolingZoneCabinet, CabinetTemperatureSensor
...
async def calculate_shift_recommendation(
    session: AsyncSession, device: PowerDevice, target_reduction_ratio: float = 0.3
) -> ShiftRecommendation:
    """
    计算设备功率转移建议（数据中心版本）
...
async def _calculate_temperature_constraint(
    session: AsyncSession, device: PowerDevice, target_reduction_ratio: float
    ...
    try:
        ...
        # 2. 查找该空调服务的制冷区域
        zone_unit_query = select(CoolingZoneUnit).where(CoolingZoneUnit.cooling_unit_id == cooling_unit.id)
        ...
        if not zone_units:
            ...
            if device.area_code:
                ...
                cabinets_query = select(Cabinet).where(Cabinet.location.like(f"%{device.area_code}%")).limit(50)
            ...
            zone_cabinet_query = select(CoolingZoneCabinet).where(CoolingZoneCabinet.zone_id.in_(zone_ids))
        ...
        for cabinet in cabinets:
            ...
            if not sensor or not sensor.point_id:
                # 尝试 outlet 或 ambient
                sensor_query = (
                    select(CabinetTemperatureSensor).where(CabinetTemperatureSensor.cabinet_id == cabinet.id).limit(1)
                )
                sensor_result = await session.execute(sensor_query)
                sensor = sensor_result.scalar_one_or_none()
            ...
            if sensor and sensor.point_id:
                ...
                latest_temp_query = (
                    select(PointHistory)
                    .where(PointHistory.point_id == sensor.point_id)
    ...
    try:
        ...
        for dev in all_devices:
            ...
            if dev.power_point_id:
                latest_power_query = (
                    select(PointHistory)
                    .where(PointHistory.point_id == dev.power_point_id)
                    .order_by(PointHistory.recorded_at.desc())
                    .limit(1)
                )
    ...
    try:
        ...
        for it_dev in it_devices:
            if it_dev.power_point_id:
                latest_power_query = (
                    select(PointHistory)
                    .where(PointHistory.point_id == it_dev.power_point_id)
                    .order_by(PointHistory.recorded_at.desc())
                    .limit(1)
                )
                latest_power_result = await session.execute(latest_power_query)
        ...
        # 2. 获取制冷负载
        cooling_devices_query = select(PowerDevice).where(PowerDevice.device_type.in_(list(COOLING_DEVICES)))
        ...
        for cooling_dev in cooling_devices:
            if cooling_dev.power_point_id:
                latest_power_query = (
                    select(PointHistory)
                    .where(PointHistory.point_id == cooling_dev.power_point_id)
                    .order_by(PointHistory.recorded_at.desc())
                    .limit(1)
                )
                latest_power_result = await session.execute(latest_power_query)
...
async def _calculate_device_constraint(
    ...
    3. 最小运行时间
    """
    constraint = {"max_reduction_ratio": target_reduction_ratio, "reason": "设备特性约束满足"}
...
async def calculate_batch_recommendations(
    session: AsyncSession, device_ids: List[int], target_reduction_ratio: float = 0.3
    ...
    for device_id in device_ids:
        ...
        if device:
            recommendation = await calculate_shift_recommendation(session, device, target_reduction_ratio)
...
async def calculate_shiftable_power_for_zone(zone_id: int, session: AsyncSession) -> Dict:
    """
...
async def _get_thm_config(session: AsyncSession) -> Dict[str, float]:
    ...
    """
    defaults = {"thm_safety_factor": 0.8, "thm_absolute_max_ratio": 0.6, "thm_min_headroom_celsius": 2.0}
...
async def _calculate_temperature_rise_rate(zone_id: int, session: AsyncSession) -> float:
    ...
    if len(data_points) < 12:
        logger.warning(
            f"Insufficient data for zone {zone_id}: {len(data_points)} points < 12, using conservative 0.5°C/h"
        )
        return 0.5
    ...
    if len(time_series) < 12:
        logger.warning(
            f"Insufficient aggregated data for zone {zone_id}: {len(time_series)} points < 12, using conservative 0.5°C/h"
        )
        return 0.5
    ...
    if len(filtered_series) < 6:
        logger.warning(
            f"Insufficient data after filtering for zone {zone_id}: {len(filtered_series)} points < 6, using conservative 0.5°C/h"
        )
        return 0.5
    ...
    if slope > 2.0 or slope < -1.0:
        logger.warning(
            f"Abnormal temperature rise rate for zone {zone_id}: {slope:.2f}°C/h, using conservative 0.5°C/h"
        )
        return 0.5
...
async def _calculate_shiftable_power_thm(zone_id: int, session: AsyncSession) -> Dict:
    ...
    if temperature_rise_rate > 0:
        ...
        if thermal_buffer_hours < cooling_lag_hours * 1.5:  # 30 分钟
            logger.warning(
                f"Zone {zone_id} thermal buffer {thermal_buffer_hours:.2f}h < {cooling_lag_hours * 1.5:.2f}h, ratio=0"
            )
            return {
                "zone_id": zone_id,
                "shiftable_ratio": 0.0,
        ...
        logger.info(
            f"Zone {zone_id} temperature rise rate {temperature_rise_rate:.3f}°C/h <= 0, skipping thermal buffer check"
        )
    ...
    logger.info(
        f"Zone {zone_id} THM calculation: headroom={headroom:.2f}°C, T_supply={T_supply:.2f}°C, "
...
async def _calculate_shiftable_power_tcl(zone_id: int, session: AsyncSession) -> Dict:
    ...
    if T_current_max is None:
        return {"error": "sensor_offline", "zone_id": zone_id, "details": "No current temperature data available"}
    ...
    if predicted_temp < T_max - 2.0:
        ratio = 0.4
        logger.info(
            f"Zone {zone_id} TCL prediction: predicted_temp={predicted_temp:.2f}°C < {T_max - 2.0}°C, ratio=0.4"
        )
    else:
        ratio = 0.0
        logger.warning(
            f"Zone {zone_id} TCL prediction: predicted_temp={predicted_temp:.2f}°C >= {T_max - 2.0}°C, ratio=0"
        )

...
class LoadForecaster:
    ...
    def forecast_day(
        self, target_date: datetime, historical_data: Optional[List[Dict]] = None, noise_level: float = 0.05
    ) -> Dict:
        """
        预测指定日期的24小时负荷曲线
        ...
        for hour in range(24):
            ...
            for quarter in range(4):
                ...
                predicted_power = (
                    self.base_load
                    + (self.peak_load - self.base_load) * base_ratio * seasonal_factor * adjustment_factor
                )
    ...
    def forecast_with_devices(
        self, target_date: datetime, base_forecast: Dict, dispatchable_devices: List[Dict]
    ) -> Dict:
        """
        考虑可调度设备的负荷预测
...
async def get_load_forecast(db: AsyncSession, target_date: datetime, meter_point_id: Optional[int] = None) -> Dict:
    """
    获取负荷预测（API服务入口）

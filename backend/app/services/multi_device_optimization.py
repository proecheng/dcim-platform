...
from .datacenter_shift_strategy import calculate_shift_recommendation, PUE_MAX
...
async def optimize_multi_device_shift(
    session: AsyncSession, device_ids: List[int], target_total_reduction: float = 0.3
) -> OptimizationResult:
    """
    多设备协同优化
    ...
    if not devices:
        return OptimizationResult(success=False, shift_plan={}, total_reduction=0.0, message="未找到设备")
...
async def _scipy_optimization(
    session: AsyncSession, devices: List[PowerDevice], target_total_reduction: float
) -> OptimizationResult:
    ...
    # 1. 总转移比例约束
    constraints.append({"type": "eq", "fun": lambda x: sum(x) - target_total_reduction * n})
    ...
    # 优化
    result = minimize(objective, x0, method="SLSQP", bounds=bounds, constraints=constraints, options={"maxiter": 100})

    if result.success:
        shift_plan = {devices[i].id: float(result.x[i]) for i in range(n)}
        ...
        return OptimizationResult(
            success=False, shift_plan={}, total_reduction=0.0, message=f"优化失败: {result.message}"
        )


async def _greedy_optimization(
    session: AsyncSession, device_ids: List[int], target_total_reduction: float
) -> OptimizationResult:
    """
    贪心算法优化（Fallback）
    ...
    for device_id in device_ids:
        ...
        device_constraints.append(
            {"device_id": device_id, "max_ratio": recommendation.recommended_ratio, "device": device}
        )
    ...
    return OptimizationResult(
        success=True, shift_plan=shift_plan, total_reduction=total_reduction, message="使用贪心算法优化（scipy不可用）"
    )

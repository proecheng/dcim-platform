...
class MILPOptimizer:
    ...
    def optimize(self, base_load: List[float], demand_target: Optional[float] = None) -> Dict[str, Any]:
        """
        执行优化
    ...
    def _optimize_milp(self, base_load: List[float], demand_target: float) -> Dict[str, Any]:
        """MILP优化实现"""
        T = self.time_slots
        ...
        if self.storage:
            P_charge = [
                pulp.LpVariable(f"P_charge_{t}", lowBound=0, upBound=self.storage.max_charge_power) for t in range(T)
            ]
            P_discharge = [
                pulp.LpVariable(f"P_discharge_{t}", lowBound=0, upBound=self.storage.max_discharge_power)
                for t in range(T)
            ]
            SOC = [
                pulp.LpVariable(f"SOC_{t}", lowBound=self.storage.min_soc, upBound=self.storage.max_soc)
                for t in range(T + 1)
            ]
            # 充放电互斥二进制变量
        ...
        for d_idx, device in enumerate(self.devices):
            if device.device_type == DeviceType.SHIFTABLE:
                # 时移型：开关状态
                device_vars[d_idx] = {"on": [pulp.LpVariable(f"dev_{d_idx}_on_{t}", cat="Binary") for t in range(T)]}
            elif device.device_type == DeviceType.CURTAILABLE:
                # 削减型：削减比例
                device_vars[d_idx] = {
                    "curtail": [
                        pulp.LpVariable(f"dev_{d_idx}_curtail_{t}", lowBound=0, upBound=device.curtail_ratio)
                        for t in range(T)
                    ]
                }
                ...
                device_vars[d_idx] = {
                    "power": [
                        pulp.LpVariable(f"dev_{d_idx}_power_{t}", lowBound=device.min_power, upBound=device.max_power)
                        for t in range(T)
                    ]
                }
        ...
        # 电量电费
        energy_cost = pulp.lpSum([self.slot_prices[t] * base_load[t] * dt for t in range(T)])

        # 储能相关成本
        if self.storage:
            # 储能放电收益（高价时段放电）
            storage_benefit = pulp.lpSum(
                [self.slot_prices[t] * P_discharge[t] * dt * self.storage.discharge_efficiency for t in range(T)]
            )
            # 储能充电成本（低价时段充电）
            storage_cost = pulp.lpSum(
                [self.slot_prices[t] * P_charge[t] * dt / self.storage.charge_efficiency for t in range(T)]
            )
            # 循环成本
            cycle_cost = pulp.lpSum([self.storage.cycle_cost * (P_charge[t] + P_discharge[t]) * dt for t in range(T)])
        ...
        if pulp.LpStatus[status] not in ["Optimal", "Not Solved"]:
            return {"status": "failed", "message": f"Solver status: {pulp.LpStatus[status]}", "optimal_value": None}
        ...
        result["saving_ratio"] = round(
            (baseline_cost - result["optimal_value"]) / baseline_cost * 100 if baseline_cost > 0 else 0, 2
        )

        return result

    def _calculate_baseline_cost(self, base_load: List[float], demand_target: float) -> float:
        """计算基线成本（不优化的情况）"""
        dt = self.slot_duration
    ...
    def _optimize_heuristic(self, base_load: List[float], demand_target: float) -> Dict[str, Any]:
        """
        启发式优化（当MILP不可用时使用）
        ...
        if self.storage:
            ...
            for t in range(T):
                ...
                if period in ["valley", "deep_valley"] and soc < self.storage.max_soc:
                    charge = min(
                        self.storage.max_charge_power, (self.storage.max_soc - soc) * self.storage.capacity / dt
                    )

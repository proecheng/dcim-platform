...
class FormulaCalculator:
    ...
    async def calc_annual_energy(self, year: int) -> Decimal:
        ...
        返回:
            年用电量 (kWh)
        """
        stmt = select(func.sum(EnergyMonthly.total_energy)).where(EnergyMonthly.stat_year == year)
    ...
    async def calc_max_demand(self, year: int, month: Optional[int] = None) -> Decimal:
        ...
        返回:
            最大需量 (kW)
        """
        stmt = select(func.max(DemandHistory.max_demand)).where(DemandHistory.stat_year == year)
    ...
    async def calc_peak_valley_data(self, start_date: date, end_date: date) -> Dict[str, Any]:
        ...
        stmt = select(func.sum(EnergyDaily.total_energy)).where(
            EnergyDaily.stat_date >= start_date, EnergyDaily.stat_date <= end_date
        )
        result = await self.db.execute(stmt)
    ...
    async def calc_shiftable_load(self, meter_point_id: Optional[int] = None) -> Decimal:
        ...
        """
        stmt = (
            select(func.sum(PowerDevice.rated_power * DeviceShiftConfig.shiftable_power_ratio))
            .select_from(PowerDevice)
            .join(DeviceShiftConfig, PowerDevice.id == DeviceShiftConfig.device_id)
            .where(DeviceShiftConfig.is_shiftable == True, PowerDevice.is_enabled == True)
        )
    ...
    async def calc_demand_control_data(self, meter_point_id: Optional[int] = None) -> Dict[str, Any]:
        ...
        demand_95th = (
            Decimal(str(demand_history.demand_95th)) if demand_history and demand_history.demand_95th else Decimal("0")
        )
        ...
        if current_declared > recommended_demand:
            monthly_saving = ((current_declared - recommended_demand) * demand_price / Decimal("10000")).quantize(
                Decimal("0.01")
            )
    ...
    async def calc_equipment_load_rate(self, equipment_type: str, start_date: date, end_date: date) -> Decimal:
        ...
        """
        stmt = (
            select(func.avg(EnergyHourly.avg_power / PowerDevice.rated_power * 100))
            .select_from(EnergyHourly)
            .join(PowerDevice, EnergyHourly.device_id == PowerDevice.id)
            .where(
    ...
    async def calc_equipment_optimization_potential(self, equipment_type: str) -> Dict[str, Decimal]:
        ...
        ).where(PowerDevice.device_type == equipment_type, PowerDevice.is_enabled == True)
    ...
    async def calc_vpp_response_potential(self) -> Dict[str, Any]:
        ...
        level1_revenue = (
            level1_capacity
            / Decimal("1000")
            * Decimal(str(level1_response_count))
            * level1_compensation
            / Decimal("10000")
        ...
        level2_revenue = (
            level2_capacity
            / Decimal("1000")
            * Decimal(str(level2_response_count))
            * level2_compensation
            / Decimal("10000")
        ).quantize(Decimal("0.01"))
        level3_revenue = (
            level3_capacity
            / Decimal("1000")
            * Decimal(str(level3_response_count))
            * level3_compensation
            / Decimal("10000")
        ).quantize(Decimal("0.01"))
    ...
    async def calc_load_curve_analysis(self, analysis_date: date) -> Dict[str, Any]:
        ...
        ).where(func.date(PowerCurveData.timestamp) == analysis_date)
    ...
    async def calc_equipment_efficiency_benchmark(self, equipment_type: str) -> Dict[str, Any]:
        ...
        stmt = select(func.avg(PowerDevice.efficiency)).where(
            PowerDevice.device_type == equipment_type, PowerDevice.is_enabled == True
        )
        result = await self.db.execute(stmt)
    ...
    async def _get_electricity_price(self, time_slot: str) -> Decimal:
        ...
        stmt = (
            ...
            .where(ElectricityPricing.period_type == time_slot, ElectricityPricing.is_enabled == True)
            .limit(1)
        )

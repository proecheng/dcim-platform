...
from .datacenter_shift_strategy import TEMP_WARNING, PUE_MAX, PUE_MIN
...
async def validate_shift_plan(
    ...
    try:
        ...
        # 2. 模拟功率转移
        projected_state = await _simulate_power_shift(session, current_state, shift_plan)
        ...
        # 6. 生成总结
        if result.is_safe:
            result.warnings.append(f"验证通过：{len(shift_plan)}个设备的功率转移计划安全")
        else:
            result.errors.append(f"验证失败：发现{len(result.errors)}个安全问题")
...
async def _load_current_state(session: AsyncSession) -> Dict:
    """加载当前系统状态"""
    state = {"devices": {}, "total_power": 0.0, "it_power": 0.0, "cooling_power": 0.0, "pue": 0.0}
...
async def _simulate_power_shift(session: AsyncSession, current_state: Dict, shift_plan: Dict[int, float]) -> Dict:
    """模拟功率转移后的状态"""
    projected_state = {
        "devices": {},
        "total_power": 0.0,
...
def _check_temperature_violations(projected_state: Dict) -> List[str]:
    ...
    for device_id, temp_rise in projected_state.get("temperature_rise", {}).items():
        ...
        if projected_temp > TEMP_WARNING:
            ...
            violations.append(
                f"设备 {device.get('name', device_id)} 预测温度 {projected_temp:.1f}℃ 超过警告阈值 {TEMP_WARNING}℃"
            )
...
def _check_redundancy_violations(projected_state: Dict) -> List[str]:
    ...
    for group_key, devices in device_groups.items():
        ...
        if total_load > n_plus_one_capacity:
            violations.append(
                f"{group_key} 设备组 N+1 冗余不足：负载 {total_load:.1f}kW > N+1容量 {n_plus_one_capacity:.1f}kW"
            )
...
def _check_pue_violations(projected_state: Dict) -> List[str]:
    ...
    if pue > PUE_MAX:
        violations.append(f"预测PUE {pue:.2f} 超过最大值 {PUE_MAX}")
    elif pue < PUE_MIN and pue > 0:
        violations.append(f"预测PUE {pue:.2f} 低于最小值 {PUE_MIN}，可能制冷不足")

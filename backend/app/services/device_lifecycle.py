...
class DeviceLifecycleService:
    ...
    async def analyze_delete_impact(self, device_id: int) -> dict:
        ...
        if point_ids:
            # 2. 点位实时数据
            rt_count = (await self.db.execute(
            ...
            # 3. 点位历史数据
            hist_count = (await self.db.execute(
                select(func.count(PointHistory.id)).where(PointHistory.point_id.in_(point_ids))
            )).scalar() or 0
            if hist_count > 0:
                impacts.append({
            ...
            # 4. 告警阈值
            threshold_count = (await self.db.execute(
        ...
        # 5. UPS 扩展
        ups_result = await self.db.execute(
            select(UPSDevice).where(UPSDevice.device_id == device_id)
        )
        ...
        if ups_devices:
            ...
            bat_count = (await self.db.execute(
        ...
        # 7. 制冷扩展
        cu_count = (await self.db.execute(
            select(func.count(CoolingUnit.id)).where(CoolingUnit.device_id == device_id)
        )).scalar() or 0
        if cu_count > 0:
            impacts.append({
        ...
        ca_count = (await self.db.execute(
            select(func.count(ColdAisle.id)).where(ColdAisle.device_id == device_id)
        )).scalar() or 0
        if ca_count > 0:
            impacts.append({
        ...
        # 9. 设备健康评分
        hs_count = (await self.db.execute(
        ...
        # 10. 三相接线映射
        pm_count = (await self.db.execute(
        ...
        # 11. 配电面板（解除关联，不删除）
        dp_count = (await self.db.execute(
        ...
        # 12. 能源拓扑设备（解除关联，不删除）
        pd_count = (await self.db.execute(
            select(func.count(PowerDevice.id)).where(PowerDevice.monitor_device_id == device_id)
        )).scalar() or 0
        if pd_count > 0:
            impacts.append({
    ...
    async def cascade_delete(self, device_id: int) -> dict:
        ...
        if point_ids:
            ...
            r = await self.db.execute(
                delete(AlarmThreshold).where(AlarmThreshold.point_id.in_(point_ids))
            )
            ...
            # 3. point_history
            r = await self.db.execute(
                delete(PointHistory).where(PointHistory.point_id.in_(point_ids))
            )
            deleted["point_history"] = r.rowcount

        # 4-5. battery_groups → ups_devices
        ups_result = await self.db.execute(
            select(UPSDevice.id).where(UPSDevice.device_id == device_id)
        )
        ups_ids = [row[0] for row in ups_result.all()]
        if ups_ids:
            r = await self.db.execute(
                delete(BatteryGroup).where(BatteryGroup.ups_device_id.in_(ups_ids))
            )
            deleted["battery_groups"] = r.rowcount

        r = await self.db.execute(
            delete(UPSDevice).where(UPSDevice.device_id == device_id)
        )
        deleted["ups_devices"] = r.rowcount

        # 6. cooling_units
        r = await self.db.execute(
            delete(CoolingUnit).where(CoolingUnit.device_id == device_id)
        )
        deleted["cooling_units"] = r.rowcount

        # 7. cold_aisles
        r = await self.db.execute(
            delete(ColdAisle).where(ColdAisle.device_id == device_id)
        )
        deleted["cold_aisles"] = r.rowcount

        # 8. device_health_scores
        r = await self.db.execute(
            delete(DeviceHealthScore).where(DeviceHealthScore.device_id == device_id)
        )
        deleted["device_health_scores"] = r.rowcount

        # 9. power_phase_mappings
        r = await self.db.execute(
            delete(PowerPhaseMapping).where(PowerPhaseMapping.pdu_device_id == device_id)
        )
        ...
        if point_ids:
            r = await self.db.execute(
                delete(Point).where(Point.device_id == device_id)
            )
        ...
        pd_result = await self.db.execute(
            select(PowerDevice).where(PowerDevice.monitor_device_id == device_id)
        )
    ...
    async def on_device_created(self, device: Device) -> None:
        ...
        if device.device_type == "UPS":
            ...
            existing = await self.db.execute(
                select(CoolingUnit).where(CoolingUnit.device_id == device.id)
            )
            if not existing.scalar_one_or_none():
            ...
            existing = await self.db.execute(
                select(ColdAisle).where(ColdAisle.device_id == device.id)
            )
            if not existing.scalar_one_or_none():
                ca = ColdAisle(
                    device_id=device.id,
    ...
    async def analyze_power_device_delete_impact(self, power_device_id: int) -> dict:
        ...
        daily_count = (await self.db.execute(
            select(func.count(EnergyDaily.id)).where(EnergyDaily.device_id == power_device_id)
        )).scalar() or 0

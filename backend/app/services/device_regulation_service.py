...
class DeviceRegulationService:
    ...
    async def get_adjustable_devices(self) -> List[Dict]:
        ...
        result = await self.db.execute(
            ...
            .where(and_(LoadRegulationConfig.is_enabled == True, PowerDevice.is_enabled == True))
            .order_by(PowerDevice.device_type, PowerDevice.device_code)
        )
    ...
    async def get_device_shift_config(self, device_id: int) -> Optional[Dict]:
        ...
        result = await self.db.execute(
            select(PowerDevice, DeviceShiftConfig)
            .outerjoin(DeviceShiftConfig, PowerDevice.id == DeviceShiftConfig.device_id)
    ...
    async def get_devices_by_type(self, device_type: str, shiftable_only: bool = False) -> List[Dict]:
        """
        按设备类型获取设备列表
        ...
        if shiftable_only:
            ...
            return [self._build_device_dict(device, shift_config) for device, shift_config in result.all()]
        else:
            result = await self.db.execute(
                select(PowerDevice).where(and_(PowerDevice.device_type == device_type, PowerDevice.is_enabled == True))
            )
            return [self._build_device_dict(device, None) for device in result.scalars().all()]
    ...
    async def get_devices_for_suggestion(self, suggestion_type: str = "peak_valley") -> List[Dict]:
        """
        根据建议类型获取相关设备列表
    ...
    async def validate_shift_constraints(self, device_ids: List[int], target_hours: List[int]) -> Dict[str, Any]:
        """
        验证设备转移时段约束
        ...
        for device_id in device_ids:
            ...
            if not config or not config.get("is_shiftable"):
                invalid_devices.append({"device_id": device_id, "reason": "设备不可转移或未配置"})
                continue
    ...
    async def get_regulation_data_source(self) -> Dict[str, Any]:
        ...
        """
        # 统计配置数量
        shift_result = await self.db.execute(select(DeviceShiftConfig).where(DeviceShiftConfig.is_shiftable == True))
        shift_count = len(shift_result.scalars().all())

        reg_result = await self.db.execute(select(LoadRegulationConfig).where(LoadRegulationConfig.is_enabled == True))
    ...
    def _calc_power_range(self, config: LoadRegulationConfig) -> Dict:
        ...
        if config.power_curve:
            ...
            try:
                ...
                return {"min_power": min(powers), "max_power": max(powers), "max_change": max(powers) - min(powers)}
    ...
    def _build_device_dict(self, device: PowerDevice, shift_config: Optional[DeviceShiftConfig]) -> Dict:
        """构建设备字典"""
        base_dict = {
            "device_id": device.id,
            "device_code": device.device_code,
        ...
        if shift_config:
            ...
            base_dict.update({"is_shiftable": False, "shiftable_power": 0, "shiftable_ratio": 0})
    ...
    async def get_device_typical_profile(self, device_id: int, days: int = 30) -> Optional[Dict[str, Any]]:
        """
        ...
        result = await self.db.execute(
            ...
            .group_by(extract("hour", EnergyHourly.stat_time))
            .order_by(extract("hour", EnergyHourly.stat_time))
        )
        ...
        # 峰时用电占比
        peak_hours_power = sum(p["avg_power"] for p in hourly_profile if p["period_type"] in ("sharp", "peak"))
    ...
    async def get_ratio_recommendations(self, days: int = 30) -> Dict[str, Any]:
        ...
        for device, shift_config in result.all():
            ...
            # 计算推荐值
            recommendation = self._calculate_recommendation(device, shift_config, hourly_stats, daily_stats)
    ...
    async def _get_hourly_stats(self, device_id: int, cutoff_date: datetime) -> Dict[str, Any]:
        ...
        result = await self.db.execute(
            select(
                ...
                func.min(EnergyHourly.min_power).label("min_power"),
                func.count(EnergyHourly.id).label("record_count"),
            ).where(and_(EnergyHourly.device_id == device_id, EnergyHourly.stat_time >= cutoff_date))
        )
        row = result.first()
    ...
    async def _get_daily_stats(self, device_id: int, cutoff_date: datetime) -> Dict[str, Any]:
        ...
        result = await self.db.execute(
            ...
            ).where(and_(EnergyDaily.device_id == device_id, EnergyDaily.stat_date >= cutoff_date.date()))
        )
        row = result.first()
    ...
    def _calculate_recommendation(
        self, device: PowerDevice, shift_config: Optional[DeviceShiftConfig], hourly_stats: Dict, daily_stats: Dict
    ) -> Dict[str, Any]:
        """计算单个设备的推荐 ratio"""
    ...
    async def update_device_ratio(self, device_id: int, new_ratio: float, auto_commit: bool = True) -> Dict[str, Any]:
        """
        ...
        if config:
            ...
            # 创建新配置
            old_ratio = 0
            config = DeviceShiftConfig(device_id=device_id, is_shiftable=new_ratio > 0, shiftable_power_ratio=new_ratio)
            self.db.add(config)

        if auto_commit:
            await self.db.commit()

        return {"device_id": device_id, "old_ratio": old_ratio, "new_ratio": new_ratio, "success": True}
    ...
    async def accept_all_recommendations(self, days: int = 30) -> Dict[str, Any]:
        ...
        if not updates:
            return {"message": "没有需要更新的设备", "updated_count": 0}

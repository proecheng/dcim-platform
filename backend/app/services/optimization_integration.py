...
from ..models.energy import EnergyOpportunity, ExecutionPlan, ExecutionResult
...
class OptimizationIntegrationService:
    ...
    async def create_opportunity_from_optimization(
        self, optimization_result: Dict, optimization_type: str = "day_ahead", target_date: Optional[str] = None
        ...
        analysis_data = {
            ...
            "device_schedule_summary": self._summarize_device_schedule(optimization_result.get("schedule", [])),
        }
    ...
    async def sync_execution_status(self, opportunity_id: int, execution_data: Dict) -> bool:
        """
        同步执行状态
        ...
        return {
            ...
            "execution": {
                ...
                "achievement_rate": round(
                    actual_saving / total_potential_saving * 100 if total_potential_saving > 0 else 0, 2
                ),
            },
        }


async def auto_generate_opportunities(db: AsyncSession, days: int = 7) -> List[EnergyOpportunity]:
    """
    自动生成节能机会
    ...
    for i in range(days):
        ...
        opportunity = await service.create_opportunity_from_optimization(
            optimization_result=result, optimization_type="day_ahead", target_date=date_str
        )

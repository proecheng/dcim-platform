...
from app.models.load_shift import ShiftPlan, ShiftExecution
...
class ShiftPlanService:
    ...
    async def get_plan(db: AsyncSession, plan_id: int) -> Optional[ShiftPlan]:
        ...
        result = await db.execute(
            select(ShiftPlan).where(ShiftPlan.id == plan_id)
        )
    ...
    async def _generate_plan_code(db: AsyncSession, shift_date: date) -> str:
        ...
        result = await db.execute(
            select(func.count(ShiftPlan.id)).where(
                ShiftPlan.plan_code.like(f"{prefix}%")
            )
        )

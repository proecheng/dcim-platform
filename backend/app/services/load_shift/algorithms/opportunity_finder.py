...
class OpportunityFinder:
    ...
    async def find_daily_opportunities(self, analysis_date: date, lookback_days: int = 30) -> List[ShiftOpportunity]:
        """
        发现指定日期的转移机会
        ...
        # 1. 分析峰谷差异
        peak_valley_analysis = await self._analyze_peak_valley_diff(analysis_date, lookback_days)
        ...
        for shift_pair in shift_pairs:
            opportunity = await self._create_opportunity(analysis_date, shift_pair, lookback_days)
            if opportunity:
                opportunities.append(opportunity)

        return opportunities

    async def _analyze_peak_valley_diff(self, target_date: date, lookback_days: int) -> Optional[Dict[str, Any]]:
        """
        分析峰谷差异
    ...
    def _identify_shift_pairs(self, analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        识别转移对（源时段 → 目标时段）
    ...
    async def _create_opportunity(
        self, analysis_date: date, shift_pair: Dict[str, Any], lookback_days: int
    ) -> Optional[ShiftOpportunity]:
        """
        创建转移机会记录
        """
        # 推荐设备
        recommended_devices = await self._recommend_shift_devices(shift_pair["recommended_power"])
    ...
    async def _recommend_shift_devices(self, target_power: float) -> List[Dict[str, Any]]:
        """
        推荐可转移设备组合
        ...
        stmt = (
            select(PowerDevice)
            .where(
                and_(PowerDevice.is_shiftable == True, PowerDevice.is_critical == False, PowerDevice.rated_power > 0)
            )
    ...
    def _calculate_predicted_saving(self, shift_power: float, price_diff: Decimal, duration_hours: int) -> Decimal:
        """
        计算预期成本节省
    ...
    def _calculate_confidence_score(self, lookback_days: int, device_count: int) -> float:
        """
        计算置信度分数
    ...
    def _determine_priority(self, predicted_saving: Decimal, confidence: float) -> str:
        """
        确定机会优先级

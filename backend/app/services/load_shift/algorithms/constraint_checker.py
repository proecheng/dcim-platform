...
from app.schemas.load_shift import ConstraintType, ConstraintCheckResult, FeasibilityAnalysisRequest
...
class ConstraintChecker:
    ...
    async def _get_dynamic_cooling_ratio(self, zone_id_raw: Any) -> Optional[float]:
        ...
        try:
            ...
            if ratio is not None:
                logger.info(
                    f"Zone {zone_id} 使用动态制冷比例: {ratio:.4f} (方法: {calc_result.get('method', 'unknown')})"
                )
                return float(ratio)
    ...
    async def check_all_constraints(
        self, request: FeasibilityAnalysisRequest, device_ids: List[int]
        ...
        power_result = await self._check_power_constraints(request, devices, device_configs, constraints)
        ...
        time_result = await self._check_time_constraints(request, devices, device_configs, constraints)
        ...
        device_result = await self._check_device_constraints(request, devices, device_configs, constraints)
        ...
        safety_result = await self._check_safety_constraints(request, devices, device_configs, constraints)
    ...
    async def _load_constraints(self) -> List[ShiftConstraint]:
        ...
        result = await self.db.execute(select(ShiftConstraint).where(ShiftConstraint.is_enabled == True))
        return result.scalars().all()
    ...
    async def _load_device_configs(self, device_ids: List[int]) -> List[DeviceShiftConfig]:
        ...
        result = await self.db.execute(select(DeviceShiftConfig).where(DeviceShiftConfig.device_id.in_(device_ids)))
        return result.scalars().all()
        ...
        for device in devices:
            ...
            if config.allowed_shift_hours:
                if shift_hour not in config.allowed_shift_hours:
                    warnings.append(f"设备 {device.device_name} 在 {shift_hour}:00 不在允许转移时段内")

            # Check forbidden_shift_hours
            if config.forbidden_shift_hours:
                if shift_hour in config.forbidden_shift_hours:
                    warnings.append(f"设备 {device.device_name} 在 {shift_hour}:00 处于禁止转移时段")
    ...
    async def _check_safety_constraints(
        ...
        # ---------- IT 负载占比 + 最大可转移功率 ----------
        merged_dc_constraints = dc_constraints + [c for c in safety_constraints if "max_it_load_ratio" in self._cfg(c)]
                ...
                violations.append(
                    {
                        ...
                        "message": (f"IT负载占比 {current_it_ratio:.2%} 不在允许区间 [{it_min:.2%}, {it_max:.2%}]"),

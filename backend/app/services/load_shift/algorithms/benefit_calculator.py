...
from app.schemas.load_shift import FeasibilityAnalysisRequest, BenefitAnalysisResponse, ShiftPeriodType
...
class BenefitCalculator:
    ...
    async def calculate_benefits(
        self, request: FeasibilityAnalysisRequest, device_ids: List[int], shift_duration_hours: float = 1.0
        ...
        cost_saving = await self._calculate_cost_saving(
            request.shift_from_period, request.shift_to_period, energy_saving, pricing
        )
    ...
    async def calculate_yearly_benefits(self, year: int, monthly_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "best_month": {"month": best_month["month"], "cost_saving": best_month["total_cost_saving_yuan"]}
            if best_month
            else None,
            "worst_month": {"month": worst_month["month"], "cost_saving": worst_month["total_cost_saving_yuan"]}
            if worst_month
            else None,

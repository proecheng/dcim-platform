    ...
    async def generate_monthly_report(
        ...
        result = await db.execute(
            select(ShiftExecution).where(
                and_(
                    ShiftExecution.start_time >= start_date,
                    ShiftExecution.start_time < end_date
                )
            )
        )
        ...
        total_cost_saving = sum(
            float(e.actual_cost_saving) if e.actual_cost_saving else 0
            for e in executions
        )
        total_energy_saving = sum(
            float(e.actual_energy_saving) if e.actual_energy_saving else 0
            for e in executions
        )
        total_shift_power = sum(
            float(e.actual_shift_power) if e.actual_shift_power else 0
            for e in executions
        )
        ...
        for date_key in sorted(daily_stats.keys()):
            ...
            stat['success_rate'] = (
                stat['success_count'] / stat['execution_count'] * 100
                if stat['execution_count'] > 0 else 0
            )
            details.append(stat)
        ...
        trend_data = [
            {
                'date': stat['date'],
                'cost_saving': stat['cost_saving'],
                'energy_saving': stat['energy_saving']
            }
            for stat in details
        ]

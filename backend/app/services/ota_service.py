...
class OtaService:
    ...
    async def create_task(
        ...
        # 验证目标网关存在且在线
        gw_result = await db.execute(select(Gateway).where(Gateway.id.in_(gateway_ids)))
        ...
        if firmware.min_version:
            incompatible = [
                gw.gateway_id for gw in gateways if gw.version and self._version_lt(gw.version, firmware.min_version)
            ]
            if incompatible:
                raise ValueError(f"网关版本不兼容 (需 >= {firmware.min_version}): {incompatible}")
        ...
        # 分配批次
        batch_assignments = self._assign_batches(gateways, strategy, batch_size, canary_percent)
    ...
    async def start_task(
        ...
        await db.execute(
            update(OtaTask).where(OtaTask.task_id == task_id).values(status="running", updated_at=datetime.now())
        )
        await db.commit()
    ...
    async def handle_ota_status(self, payload: dict, db: AsyncSession) -> None:
        ...
        if status == "success":
            ...
            # 更新网关版本
            task_result = await db.execute(select(OtaTask).where(OtaTask.task_id == task_id))
            task = task_result.scalar_one_or_none()
            if task:
                await db.execute(
                    update(Gateway)
                    .where(Gateway.gateway_id == gw_id)
                    .values(
    ...
    async def cancel_task(
        ...
        for tg in pending_gws:
            gw_result = await db.execute(select(Gateway).where(Gateway.gateway_id == tg.gateway_id))
            ...
            await db.execute(
                update(OtaTaskGateway)
                .where(OtaTaskGateway.id == tg.id)
                .values(status="cancelled", error_message="任务已取消", completed_at=datetime.now())
            )

        await db.execute(
            update(OtaTask).where(OtaTask.task_id == task_id).values(status="cancelled", updated_at=datetime.now())
        )
        await db.commit()
    ...
    async def pause_task(self, task_id: str, db: AsyncSession) -> None:
        ...
        await db.execute(
            update(OtaTask).where(OtaTask.task_id == task_id).values(status="paused", updated_at=datetime.now())
        )
        await db.commit()
    ...
    async def _dispatch_batch(
        ...
        fw_result = await db.execute(select(FirmwarePackage).where(FirmwarePackage.id == task.firmware_id))
    ...
    async def _finalize_task(self, task_id: str, db: AsyncSession) -> None:
        ...
        await db.execute(
            update(OtaTask).where(OtaTask.task_id == task_id).values(status=final_status, updated_at=datetime.now())
        )
        await db.commit()
        logger.info(
    ...
    async def _find_next_pending_batch(self, task_id: str, db: AsyncSession) -> Optional[int]:

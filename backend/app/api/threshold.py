...
from ..schemas import AlarmThresholdCreate, AlarmThresholdUpdate, AlarmThresholdResponse
...
async def get_thresholds(
    point_id: int = None, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """获取阈值配置列表"""
...
async def get_point_thresholds(
    ...
    """获取点位的阈值配置"""
    result = await db.execute(select(AlarmThreshold).where(AlarmThreshold.point_id == point_id))
    return result.scalars().all()


@router.post("", response_model=AlarmThresholdResponse)
async def create_threshold(
    data: AlarmThresholdCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """创建阈值配置"""
    # 验证点位存在
...
async def delete_threshold(
    threshold_id: int, db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """删除阈值配置"""
    result = await db.execute(select(AlarmThreshold).where(AlarmThreshold.id == threshold_id))
    threshold = result.scalar_one_or_none()
...
async def batch_create_thresholds(
    data: List[AlarmThresholdCreate], db: AsyncSession = Depends(get_db), current_user: User = Depends(get_current_user)
):
    """批量创建阈值配置"""
    created = []
    for item in data:

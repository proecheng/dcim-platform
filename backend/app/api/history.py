...
async def get_point_trend(
    ...
    for record in records:
        bucket_time = record.recorded_at.replace(second=0, microsecond=0)
        # 按分钟聚合
        bucket_key = bucket_time.replace(minute=(bucket_time.minute // (interval // 60)) * (interval // 60))
...
async def export_history_csv(
    ...
    # 查询点位信息
    points_result = await db.execute(select(Point).where(Point.id.in_(point_id_list)))
    ...
    return StreamingResponse(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": f"attachment; filename=history_{datetime.now().strftime('%Y%m%d%H%M%S')}.csv"},
    )

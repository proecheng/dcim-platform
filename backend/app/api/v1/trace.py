...
from app.schemas.trace import DataSourceMappingCreate, TemplateParameterCreate
from app.models.trace import DataSourceMapping, TraceRecord, TemplateParameter
...
async def get_trace_detail(trace_id: str, db: AsyncSession = Depends(get_db)):
    ...
    if not trace:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"追溯记录不存在: {trace_id}")

    return {"code": 0, "message": "success", "data": _trace_to_dict(trace)}
...
async def get_proposal_traces(
    ...
    if not traces:
        return {
            "code": 0,
            "message": "success",
            "data": {"total_traces": 0, "data_sources": [], "root_trace_ids": [], "traces": []},
        }
...
async def get_measure_traces(measure_id: int, db: AsyncSession = Depends(get_db)):
    """
    获取某个措施的所有追溯记录
    ...
    return {
        "code": 0,
        "message": "success",
        "data": {"measure_id": measure_id, "total": len(traces), "traces": [_trace_to_dict(t) for t in traces]},
    }
...
async def get_proposal_ml_traces(
    ...
    query = select(TraceRecord).where(
        and_(TraceRecord.proposal_id == proposal_id, TraceRecord.mapping_type == "ml_prediction")
    )
    ...
    return {
        "code": 0,
        "message": "success",
        "data": {"proposal_id": proposal_id, "summary": summary, "traces_by_model": grouped},
    }


@router.get("/measure/{measure_id}/ml", summary="获取措施ML预测追溯", response_model=None)
async def get_measure_ml_traces(measure_id: int, db: AsyncSession = Depends(get_db)):
    """
    获取措施的 ML 预测追溯记录
    ...
    """
    query = select(TraceRecord).where(
        and_(TraceRecord.measure_id == measure_id, TraceRecord.mapping_type == "ml_prediction")
    )
    result = await db.execute(query)
    traces = result.scalars().all()
...
async def list_mappings(
    ...
    # 统计总数
    count_query = select(func.count()).select_from(DataSourceMapping).where(DataSourceMapping.is_enabled == True)
    if mapping_type:
...
async def create_mapping(data: DataSourceMappingCreate, db: AsyncSession = Depends(get_db)):
    """创建新的数据源映射配置"""
    # 检查是否已存在
    existing = await db.execute(select(DataSourceMapping).where(DataSourceMapping.param_code == data.param_code))
    if existing.scalar_one_or_none():
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"参数编码已存在: {data.param_code}")
    ...
    return {"code": 0, "message": "创建成功", "data": _mapping_to_dict(mapping)}
...
async def get_template_params(template_id: str, db: AsyncSession = Depends(get_db)):
    """获取模板的占位符参数及映射配置"""
    service = DataTraceService(db)
...
async def create_template_param(data: TemplateParameterCreate, db: AsyncSession = Depends(get_db)):
    """创建模板参数配置"""
    ...
    return {"code": 0, "message": "创建成功", "data": _template_param_to_dict(param)}
...
async def init_default_mappings(db: AsyncSession = Depends(get_db)):
    """
    初始化系统默认的数据源映射配置
    ...
    return {"code": 0, "message": f"初始化完成，创建了 {created} 个映射配置", "data": {"created_count": created}}
...
async def _create_default_mappings(db: AsyncSession) -> int:
    ...
    for mapping_data in default_mappings:
        ...
        existing = await db.execute(
            select(DataSourceMapping).where(DataSourceMapping.param_code == mapping_data["param_code"])
        )
        if existing.scalar_one_or_none():
            continue

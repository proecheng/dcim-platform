...
async def get_operation_logs(
    ...
    if keyword:
        query = query.where((OperationLog.target_name.contains(keyword)) | (OperationLog.remark.contains(keyword)))
    ...
    return PageResponse(
        items=[OperationLogInfo.model_validate(log) for log in logs], total=total, page=page, page_size=page_size
    )
...
async def get_system_logs(
    ...
    return PageResponse(
        items=[SystemLogInfo.model_validate(log) for log in logs], total=total, page=page, page_size=page_size
    )
...
async def get_communication_logs(
    ...
    return PageResponse(
        items=[CommunicationLogInfo.model_validate(log) for log in logs], total=total, page=page, page_size=page_size
    )
...
async def export_logs(
    ...
    if log_type == "operation":
        query = (
            select(OperationLog)
            .where(and_(OperationLog.created_at >= start_time, OperationLog.created_at <= end_time))
        ...
        for log in logs:
            writer.writerow(
                [log.created_at, log.username, log.module, log.action, log.target_name, log.ip_address, log.remark]
            )

    elif log_type == "system":
        query = (
            select(SystemLog)
            .where(and_(SystemLog.created_at >= start_time, SystemLog.created_at <= end_time))
        ...
        for log in logs:
            writer.writerow([log.created_at, log.log_level, log.module, log.message, log.exception])

    elif log_type == "communication":
        query = (
            select(CommunicationLog)
            .where(and_(CommunicationLog.created_at >= start_time, CommunicationLog.created_at <= end_time))
...
async def get_log_statistics(
    days: int = Query(7, ge=1, le=30), db: AsyncSession = Depends(get_db), _: User = Depends(require_admin)
):
    """
    获取日志统计信息
    """
    ...
    # 操作日志统计
    op_total_result = await db.execute(select(func.count(OperationLog.id)).where(OperationLog.created_at >= start_time))
    ...
    sys_by_level_result = await db.execute(
        select(SystemLog.log_level, func.count(SystemLog.id))
    ...
    comm_by_status_result = await db.execute(
        ...
        .where(CommunicationLog.created_at >= start_time)
    ...
    return {
        "period_days": days,
        "operation_logs": {"total": op_total, "by_module": op_by_module},
        "system_logs": {"by_level": sys_by_level},
        "communication_logs": {"total": comm_total, "by_status": comm_by_status},
    }

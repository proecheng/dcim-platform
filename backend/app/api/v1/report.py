...
async def get_templates(
    template_type: Optional[str] = Query(None), db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)
):
    """
    获取报表模板列表
    """
...
async def create_template(
    data: ReportTemplateCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_operator)
):
    """
    创建报表模板
    """
...
async def update_template(
    ...
    await db.execute(update(ReportTemplate).where(ReportTemplate.id == template_id).values(**update_data))
    await db.commit()
...
async def delete_template(template_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(require_operator)):
    """
    删除报表模板
    """
    result = await db.execute(select(ReportTemplate).where(ReportTemplate.id == template_id))
...
async def generate_report(
    data: ReportGenerate, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_operator)
):
    """
    生成报表
    """
    ...
    # 获取模板
    if data.template_id:
        template_result = await db.execute(select(ReportTemplate).where(ReportTemplate.id == data.template_id))
    ...
    alarm_result = await db.execute(
        ...
        .where(and_(Alarm.created_at >= start_time, Alarm.created_at <= end_time))
    ...
    return {"record_id": record.id, "report_name": report_name, "data": report_data}
...
async def get_records(
    ...
    return PageResponse(
        items=[ReportRecordInfo.model_validate(r) for r in records], total=total, page=page, page_size=page_size
    )
...
async def download_report(
    ...
    if format == "pdf":
        ...
        report_data = {
            ...
            "period": f"{record.start_time.strftime('%Y-%m-%d')} 至 {record.end_time.strftime('%Y-%m-%d')}"
        ...
        if record.report_type == "daily":
            ...
            report_data["alarms"] = {"紧急": 0, "重要": 1, "一般": 2, "提示": 5}
...
async def get_daily_report(
    ...
    for point in points:
        stats_result = await db.execute(
            select(func.min(PointHistory.value), func.max(PointHistory.value), func.avg(PointHistory.value)).where(
                and_(
...
async def get_weekly_report(
    ...
    for i in range(7):
        ...
        count_result = await db.execute(
            select(func.count(Alarm.id)).where(and_(Alarm.created_at >= day_start, Alarm.created_at < day_end))
        )
...
async def _collect_alarm_trends(db: AsyncSession, start_time: datetime, end_time: datetime) -> dict:
    ...
    top_result = await db.execute(
        select(Alarm.point_id, func.count(Alarm.id).label("cnt"))
        .where(and_(Alarm.created_at >= start_time, Alarm.created_at < end_time))
    ...
    for row in top_result.all():
        if row[0]:
            ...
            top_points.append(
                {"point_id": row[0], "point_name": point.point_name if point else str(row[0]), "count": row[1]}
            )

    # 平均处理时间
    avg_result = await db.execute(
        select(func.avg(Alarm.duration_seconds)).where(
            and_(Alarm.created_at >= start_time, Alarm.created_at < end_time, Alarm.status == "resolved")
        )
    )
...
async def _collect_workorder_stats(db: AsyncSession, start_time: datetime, end_time: datetime) -> dict:
    ...
    status_result = await db.execute(
        ...
        .where(and_(WorkOrder.created_at >= start_time, WorkOrder.created_at < end_time))
    ...
    type_result = await db.execute(
        select(WorkOrder.order_type, func.count(WorkOrder.id))
        .where(and_(WorkOrder.created_at >= start_time, WorkOrder.created_at < end_time))
...
async def _collect_comparison(db: AsyncSession, start_time: datetime, end_time: datetime) -> dict:
    ...
    current_result = await db.execute(
        select(func.count(Alarm.id)).where(and_(Alarm.created_at >= start_time, Alarm.created_at < end_time))
    )
    ...
    prev_end = start_time
    prev_result = await db.execute(
        select(func.count(Alarm.id)).where(and_(Alarm.created_at >= prev_start, Alarm.created_at < prev_end))
    )
    ...
    yoy_result = await db.execute(
        select(func.count(Alarm.id)).where(and_(Alarm.created_at >= yoy_start, Alarm.created_at < yoy_end))
    )
...
async def auto_generate_report(
    data: AutoReportRequest, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_operator)
):
    """
...
async def get_schedules(db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
    """获取报表调度配置列表"""
...
async def create_schedule(
    data: ReportScheduleCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_operator)
):
    """创建报表调度配置"""
    if data.report_type not in VALID_REPORT_TYPES:
        raise HTTPException(status_code=422, detail=f"无效的报表类型，可选: {', '.join(VALID_REPORT_TYPES)}")
    ...
    schedule = ReportSchedule(
        name=data.name, report_type=data.report_type, is_enabled=data.is_enabled, created_by=current_user.id
    )
    db.add(schedule)
    await db.commit()
...
async def update_schedule(
    ...
    update_data["updated_at"] = datetime.now()
    await db.execute(update(ReportSchedule).where(ReportSchedule.id == schedule_id).values(**update_data))
    await db.commit()
...
async def delete_schedule(schedule_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(require_operator)):
    """删除报表调度配置"""
    result = await db.execute(select(ReportSchedule).where(ReportSchedule.id == schedule_id))
    if not result.scalar_one_or_none():
        raise HTTPException(status_code=404, detail="调度配置不存在")
...
async def get_summary_panel(db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
    """
    ...
    approval_count = (
        await db.execute(
            select(func.count(WorkOrderApproval.id)).where(WorkOrderApproval.status == ApprovalStatus.pending)
        )
    ).scalar() or 0
    if approval_count > 0:
        items.append(
    ...
    # 3. 逾期巡检任务 (priority 2)
    overdue_count = (
        await db.execute(select(func.count(InspectionTask.id)).where(InspectionTask.status == InspectionStatus.overdue))
    ).scalar() or 0
    if overdue_count > 0:
        items.append(
            {
    ...
    return {"items": items, "total_items": len(items), "generated_at": datetime.now().isoformat()}
...
async def export_auto_report_pdf(record_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
    """

...
async def get_overview(response: Response, db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
    ...
    point_enabled = (await db.execute(select(func.count(Point.id)).where(Point.is_enabled == True))).scalar()

    # 设备统计
    device_total = (await db.execute(select(func.count(Device.id)))).scalar()
    device_online = (await db.execute(select(func.count(Device.id)).where(Device.status == "online"))).scalar()

    # 活动告警
    alarm_active = (
        await db.execute(select(func.count(Alarm.id)).where(Alarm.status.in_(["active", "acknowledged"])))
    ).scalar()

    # 今日告警
    today_start = datetime.now().replace(hour=0, minute=0, second=0, microsecond=0)
    alarm_today = (await db.execute(select(func.count(Alarm.id)).where(Alarm.created_at >= today_start))).scalar()
...
async def get_points_statistics(
    ...
    # 按区域统计
    area_result = await db.execute(select(Point.area_code, func.count(Point.id)).group_by(Point.area_code))
...
async def get_alarms_statistics(
    ...
    avg_duration_result = await db.execute(
        select(func.avg(Alarm.duration_seconds)).where(and_(Alarm.created_at >= start_time, Alarm.status == "resolved"))
    )
...
async def get_comparison_data(
    ...
    if metric == "alarm":
        # 本周告警数
        this_week_result = await db.execute(select(func.count(Alarm.id)).where(Alarm.created_at >= this_week_start))
        ...
        # 本月告警数
        this_month_result = await db.execute(select(func.count(Alarm.id)).where(Alarm.created_at >= this_month_start))

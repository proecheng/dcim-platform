...
from ...schemas.energy import (
    ...
    TopologyExport,
    TopologyImport,
    DevicePointConfigCreate,
    DevicePointConfigUpdate,
...
async def create_topology_node(data: TopologyNodeCreate, db: AsyncSession = Depends(get_db)):
    """
    ...
    try:
        ...
        await sync_service.notify_data_simulator(
            ...
            node_id=node_id,
            data={
                "code": data.transformer_code
                or data.meter_code
                or data.panel_code
                or data.circuit_code
                or data.device_code
            },
        )

        return {"success": True, "node_id": node_id, "node_type": node_type, "message": "节点创建成功"}
...
async def update_topology_node(data: TopologyNodeUpdate, db: AsyncSession = Depends(get_db)):
    """
    更新拓扑节点信息
    """
    try:
...
async def delete_topology_node(data: TopologyNodeDelete, db: AsyncSession = Depends(get_db)):
    """
    删除拓扑节点，可选择级联删除子节点
    """
    try:
        ...
        sync_service = TopologySyncService(db)
        await sync_service.notify_data_simulator(
            operation="delete", node_type=data.node_type.value, node_id=data.node_id
        )

        return {"success": True, "deleted": deleted, "message": "节点删除成功"}
    except Exception as e:
        await db.rollback()
...
async def batch_topology_operation(data: TopologyBatchOperation, db: AsyncSession = Depends(get_db)):
    """
    批量执行拓扑操作（创建、更新、删除）
    支持事务，全部成功或全部回滚
    """
...
async def import_topology(data: TopologyImport, db: AsyncSession = Depends(get_db)):
    """
    导入拓扑数据
    可选择是否清除现有数据
    """
    ...
    if not result.success:
        raise HTTPException(status_code=400, detail={"message": "导入失败", "errors": result.errors})
...
async def create_device_points(data: DevicePointConfigCreate, db: AsyncSession = Depends(get_db)):
    """
    为设备批量创建测点配置
    测点会自动同步到点位管理模块
    """
    try:
        ...
        point_ids = await sync_service.sync_device_points(
            device_id=data.energy_device_id, device_code=device.device_code, points_config=data.points
        )
...
async def get_device_points(device_id: int, db: AsyncSession = Depends(get_db)):
    """
    获取指定设备的所有测点配置
    """
    from sqlalchemy import select

    result = await db.execute(select(Point).where(Point.energy_device_id == device_id))
...
async def update_device_point(point_id: int, data: DevicePointConfigUpdate, db: AsyncSession = Depends(get_db)):
    """
    更新设备测点配置
    """
    from sqlalchemy import select
    ...
    try:
        ...
        return {"success": True, "point_id": point_id, "message": "测点配置更新成功"}
    except HTTPException:
        await db.rollback()
        raise
    except Exception as e:
...
async def delete_device_points(device_id: int, db: AsyncSession = Depends(get_db)):
    """
    删除设备的所有测点配置
    """
    try:
        ...
        return {"success": True, "device_id": device_id, "deleted_count": deleted_count, "message": "测点删除成功"}
    except Exception as e:
        await db.rollback()
        raise HTTPException(status_code=500, detail=f"删除失败: {str(e)}")
...
async def delete_single_point(
    point_id: int = Path(..., gt=0, description="点位ID，必须大于0"), db: AsyncSession = Depends(get_db)
):
    """
    删除单个点位
    """
    ...
    try:
        ...
        return {"success": True, "point_id": point_id, "message": "点位删除成功"}
    except HTTPException:
        await db.rollback()
        raise
    except Exception as e:
...
async def create_topology_connection(
    ...
    try:
        ...
        if source_type == TopologyNodeType.TRANSFORMER and target_type == TopologyNodeType.METER_POINT:
            ...
            raise HTTPException(status_code=400, detail=f"不支持的连接类型: {source_type} -> {target_type}")
...
async def delete_topology_connection(
    ...
    try:
        ...
        if target_type == TopologyNodeType.METER_POINT:
            ...
            raise HTTPException(status_code=400, detail=f"不支持的节点类型: {target_type}")
...
async def sync_device_point_relations(db: AsyncSession = Depends(get_db)):
    """
    手动触发设备与点位的双向同步
...
async def get_device_linked_points(device_id: int, db: AsyncSession = Depends(get_db)):
    """
    ...
    """
    # 获取设备信息
    device_result = await db.execute(select(PowerDevice).where(PowerDevice.id == device_id))
    ...
    reverse_linked_result = await db.execute(select(Point.id).where(Point.energy_device_id == device_id))

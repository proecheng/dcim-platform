...
from ...schemas.energy import (
    ...
    CreateLoadShiftPlanRequest,
    CreateLoadShiftPlanResponse,
)
...
@router.post(
    "/plans/from-shift", response_model=ResponseModel[CreateLoadShiftPlanResponse], summary="从负荷转移配置创建执行计划"
)
...
async def list_plans(
    ...
    try:
        ...
        return ResponseModel(
            code=0, message="success", data={"items": response_items, "total": total, "skip": skip, "limit": limit}
        )
    except Exception as e:
...
async def get_plan(plan_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
    ...
    return ResponseModel(code=0, message="success", data=plan_data)


@router.put("/plans/{plan_id}/status", summary="更新计划状态")
async def update_plan_status(
    plan_id: int, request: UpdatePlanStatusRequest, db: AsyncSession = Depends(get_db), _: User = Depends(require_admin)
):
    """手动更新执行计划状态"""
...
async def complete_task(
    ...
    result = await service.complete_manual_task(
        task_id, completed_by=request.completed_by or current_user.username, notes=request.notes
    )
...
async def get_task(task_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
    """获取单个任务的详细信息"""
    from sqlalchemy import select
...
async def get_tracking(plan_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
    """
    获取执行计划的效果追踪数据
...
async def create_tracking(
    plan_id: int, request: TrackingRequest, db: AsyncSession = Depends(get_db), _: User = Depends(require_admin)
):
    """
    创建效果追踪任务
    ...
    return {"message": "追踪任务创建成功", "tracking": tracking}
...
async def get_execution_stats(db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
    """
    获取执行统计汇总
    ...
    plan_stats_result = await db.execute(
        select(ExecutionPlan.status, func.count(ExecutionPlan.id), func.sum(ExecutionPlan.expected_saving)).group_by(
    ...
    for status, count, saving in plan_stats:
        ...
        stats_by_status[status] = {"count": count, "expected_saving": saving_value}
    ...
    result_stats = await db.execute(
        select(func.sum(ExecutionResult.actual_saving), func.count(ExecutionResult.id)).where(
    ...
    if completed_tracking_count and completed_tracking_count > 0:
        ...
        tracked_plans_result = await db.execute(
            select(func.sum(ExecutionPlan.expected_saving)).where(
                ExecutionPlan.id.in_(
                    select(ExecutionResult.plan_id).where(ExecutionResult.status == "completed").distinct()
                )
            )
        )

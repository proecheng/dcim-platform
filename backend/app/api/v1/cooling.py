...
from ...schemas.cooling import (
    CoolingUnitCreate, CoolingUnitUpdate, CoolingUnitInfo, CoolingUnitDetail,
    CoolingGroupCreate, CoolingGroupUpdate, CoolingGroupInfo,
    ColdAisleCreate, ColdAisleUpdate, ColdAisleInfo, ColdAisleDetail,
...
async def get_cooling_overview(
    ...
    ac_result = await db.execute(
        ...
        .where(Device.device_type.in_([
            "PRECISION_AC_INDOOR", "PRECISION_AC_OUTDOOR"
        ]))
    ...
    group_result = await db.execute(
        select(CoolingGroup.group_mode, func.count(CoolingGroup.id))
...
async def get_cooling_units(
    ...
    query = select(CoolingUnit, Device, CoolingGroup).join(
        Device, CoolingUnit.device_id == Device.id
    ).outerjoin(
        CoolingGroup, CoolingUnit.group_id == CoolingGroup.id
    )
...
async def get_cooling_unit(
    ...
    result = await db.execute(
        select(CoolingUnit, Device, CoolingGroup).join(
            Device, CoolingUnit.device_id == Device.id
        ).outerjoin(
            CoolingGroup, CoolingUnit.group_id == CoolingGroup.id
...
async def get_cooling_groups(
    ...
    return PageResponse(
        items=[CoolingGroupInfo.model_validate(g) for g in groups],
        total=total,
        page=page,
        page_size=page_size
    )
...
async def delete_cooling_group(
    group_id: int,
    db: AsyncSession = Depends(get_db),
    _: User = Depends(require_admin)
):
    ...
    unit_count = await db.execute(
        select(func.count()).where(CoolingUnit.group_id == group_id)
    )
...
async def get_cold_aisle(
    ...
    result = await db.execute(
        select(ColdAisle, Device).join(
            Device, ColdAisle.device_id == Device.id
        ).where(ColdAisle.id == aisle_id)
    )
    row = result.first()
    if not row:

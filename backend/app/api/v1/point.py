...
async def get_points(
    ...
    if energy_device_ids:
        device_result = await db.execute(
            select(PowerDevice.id, PowerDevice.device_name, PowerDevice.device_code).where(
                PowerDevice.id.in_(energy_device_ids)
            )
        )
...
async def link_point_to_device(
    ...
    return {"message": "关联成功", "point_id": point_id, "energy_device_id": energy_device_id, "detected_usage": usage}
...
async def unlink_point_from_device(
    ...
    return {"message": "取消关联成功", "point_id": point_id, "removed_from_device_id": original_device_id}

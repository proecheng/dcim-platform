...
from ...schemas.energy import PricingConfigCreate, PricingConfigUpdate, PricingConfigResponse
...
class EstimateSavingsRequest(BaseModel):
    ...
    current_energy_by_period: Dict[str, float] = Field(..., description="当前各时段用电量 kWh")
    current_max_demand: float = Field(..., description="当前最大需量 kW")
    optimized_energy_by_period: Dict[str, float] = Field(..., description="优化后各时段用电量 kWh")
...
async def get_full_pricing_config(db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
    """
    获取完整的电价配置，包括：
    - 时段电价（尖峰/高峰/平段/低谷/深谷）
...
async def get_global_config(db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
...
async def create_global_config(
    data: PricingConfigCreate, db: AsyncSession = Depends(get_db), current_user: User = Depends(require_admin)
):
    """创建新的全局电价配置"""
    service = PricingService(db)
    config = await service.update_pricing_config(config_id=None, config_data=data.model_dump())
    return {"message": "创建成功", "config": PricingConfigResponse.model_validate(config)}
...
async def update_global_config(
    ...
    try:
        config = await service.update_pricing_config(
            config_id=config_id, config_data=data.model_dump(exclude_unset=True)
        )
    except ValueError as e:
        raise HTTPException(status_code=404, detail=str(e))

    return {"message": "更新成功", "config": PricingConfigResponse.model_validate(config)}
...
async def calculate_bill(
    request: CalculateBillRequest, db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)
):
    """
    计算完整电费账单，包括：
    - 电度电费（按时段电价计算）
...
async def estimate_savings(
    request: EstimateSavingsRequest, db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)
):
    """
...
async def get_time_period_prices(db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
    """获取所有时段电价配置"""
    service = PricingService(db)
...
async def get_peak_valley_spread(db: AsyncSession = Depends(get_db), _: User = Depends(require_viewer)):
    """获取峰谷电价差分析"""
    service = PricingService(db)

...
class StateSpace:
    ...
    def total_dim(self) -> int:
        return (self.load_data_dim + self.price_period_dim + 
                self.measure_state_dim + self.device_param_dim + 
                2 + self.history_dim)  # +2 for cumulative_saving and current_rate
...
class EnergySavingEnv:
    ...
    def _build_state(self) -> np.ndarray:
        ...
        state = np.concatenate([
            load_data, price_period, measure_states, 
            device_params, cumulative, current_rate, history
        ])
    ...
    def step(self, action: Dict[str, Any]) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        reward = (achievement_rate - 
                  ...
                  self.lambda_safety * safety_violation)
    ...
    def get_action_spec(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'discrete': {
                'target_period': self.action_space.num_target_periods
            }
        }

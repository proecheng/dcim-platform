...
from ..config import TransformerConfig, MLConfig
...
class TransferabilityPredictor:
    ...
    def predict(
        ...
        period_types: np.ndarray,
        is_weekday: np.ndarray,
        ...
        dataset = LoadTimeSeriesDataset(
            power_series, period_types, is_weekday, temperature,
        ...
        for i in range(len(dataset)):
            ...
            results.append({
                ...
                'period_probs': {
                    name: float(prob) 
                    for name, prob in zip(period_names, period_probs)
                },

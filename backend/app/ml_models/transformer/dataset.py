...
class LoadTimeSeriesDataset(Dataset):
    ...
    def __init__(
        ...
        actual_lens = np.minimum(
            ...
            if power_data.ndim == 1 else 
            np.full(len(power_data), power_data.shape[-1]),
            max_seq_len
        )
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.labels is not None:
            item["transferability"] = torch.tensor(
                self.labels["transferability"][idx], dtype=torch.float32
            ).unsqueeze(0)
            item["capacity"] = torch.tensor(
                self.labels["capacity"][idx], dtype=torch.float32
            ).unsqueeze(0)
            item["period_labels"] = torch.tensor(
                self.labels["period_labels"][idx], dtype=torch.long
            )
...
def generate_synthetic_dataset(
    ...
    train_dataset = LoadTimeSeriesDataset(
        power_data[:split_idx], period_types[:split_idx],
        is_weekday_data[:split_idx], temperature_data[:split_idx],
        labels=labels_train, max_seq_len=seq_len
    )
    
    val_dataset = LoadTimeSeriesDataset(
        power_data[split_idx:], period_types[split_idx:],
        is_weekday_data[split_idx:], temperature_data[split_idx:],
        labels=labels_val, max_seq_len=seq_len
    )

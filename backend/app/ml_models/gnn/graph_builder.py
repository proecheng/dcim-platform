...
class MeasureGraphBuilder:
    ...
    def encode_measures(self, measures: List[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if len(benefits_array) > 0 and benefits_array.max() != benefits_array.min():
            norm_benefits = (benefits_array - benefits_array.min()) / (
                benefits_array.max() - benefits_array.min()
            )
        else:
        ...
        # 措施类型名称到索引的映射
        type_map = {
            'load_shift': 0, 'load_shed': 1, 'generation': 2,
            'storage_charge': 3, 'storage_discharge': 4, 'demand_response': 5
        }
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                if same_device or time_overlap:
                    ...
                    targets.extend([j, i])
                    edge_types.extend([
                        self.EDGE_TYPE_RESOURCE_SHARING,
                        self.EDGE_TYPE_RESOURCE_SHARING
                    ])
                ...
                if has_coupling:
                    ...
                    targets.extend([j, i])
                    edge_types.extend([
                        self.EDGE_TYPE_BENEFIT_COUPLING,
                        self.EDGE_TYPE_BENEFIT_COUPLING
                    ])

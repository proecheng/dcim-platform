...
class ShiftExecution(Base):
    ...
    # 执行状态
    status = Column(String(20), default="pending", comment="状态: pending/executing/completed/failed/cancelled/reverted")
    ...
    # 设备执行详情 (JSON)
    device_execution_details = Column(JSON, comment="""设备执行详情 JSON:
        [
            {
                "device_id": 123,
                "device_name": "空调-1",
    ...
    # 制冷联动记录
    cooling_linkage_data = Column(JSON, comment="""制冷联动数据 JSON:
        {
...
class ShiftConstraint(Base):
    ...
    constraint_type = Column(String(30), nullable=False, comment="约束类型: power/time/device/cooling/safety/electrical")
    ...
    # 约束参数 (JSON)
    constraint_params = Column(JSON, nullable=False, comment="""约束参数 JSON:
        # 功率约束
        {
            "type": "power",
            "max_shift_power": 500,
...
class ShiftOpportunity(Base):
    ...
    # 分析数据 (JSON)
    analysis_data = Column(JSON, comment="""分析数据 JSON:
        {
...
class ShiftAnalysisRecord(Base):
    ...
    # 分析详情 (JSON)
    analysis_details = Column(JSON, comment="""分析详情 JSON:
        {
            "constraint_checks": [
                {

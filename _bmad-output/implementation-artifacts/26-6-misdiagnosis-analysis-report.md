...
**功能验收**:
- [ ] APScheduler 月度定时任务在每月1日凌晨自动触发（UTC 时区）
- [ ] 生成 Markdown 格式误判分析报告，包含以下内容：
...
- [ ] 报告存储在 `report_records` 表（report_type='diagnosis_monthly'）
- [ ] 防止重复生成：检查相同周期的报告是否已存在
- [ ] 生成后通知管理员查看（通过 WebSocket 系统通知，邮件通知可选）
- [ ] 管理员可通过 API 查询历史报告列表
- [ ] 管理员可通过 API 下载指定报告的 Markdown 文件

**性能验收**:
- [ ] 报告生成耗时 < 60 秒（PostgreSQL，10 万条诊断记录）
- [ ] 报告生成耗时 < 120 秒（SQLite，10 万条诊断记录）
- [ ] SQL 聚合查询使用索引优化（见 Dev Notes 4.3）

**安全验收**:
- [ ] 报告查询 API 按 RBAC 权限控制（仅管理员可访问）
- [ ] 报告生成记录审计日志
- [ ] Markdown 文件路径使用绝对路径，防止路径遍历攻击

**测试验收**:
- [ ] 单元测试覆盖率 ≥ 80%
- [ ] 集成测试覆盖核心场景（报告生成、数据统计、API 查询）
- [ ] 测试 SQLite 和 PostgreSQL 两种数据库

...
#### D39-01 已记录决策（2026-08-14）

唯一维护者 `proecheng` 记录以下生产恢复目标；不要求 Charlie、Dana 或其他 BMAD 虚拟角色审批：

| 场景 | RPO | RTO | 验收边界 |
|------|-----|-----|----------|
| 误删、损坏或错误迁移后的 PITR | `<= 5 分钟` | `<= 4 小时` | 从异故障域仓库恢复完整 PostgreSQL cluster 到指定 time/LSN/restore point；结果不得外推到未实测的数据规模 |
| 计划内主备切换 | `0` | `<= 15 分钟` | 确认 replay LSN 追平、隔离旧主、提升备库并完成应用关键读写验证 |
| 主库或主机意外故障的温备切换 | `<= 60 秒` | `<= 30 分钟` | 异步物理流复制；切换前必须 fence 旧主，禁止双主 |
| 整站丢失后的异地恢复 | `<= 5 分钟` | `<= 4 小时` | 在独立故障域重建数据库与应用连接，并通过 readiness、关键读写和一致性检查 |
| 受控迁移或应用回滚 | `0` | `<= 60 分钟` | 维护窗口停止业务写入；优先执行已验证的可逆迁移，否则恢复迁移前 restore point 和已验证应用镜像 |

- 备份策略为每周 full、每日 differential/incremental 和连续 WAL 归档；必须维持至少 35 天 PITR 窗口，并保留至少 5 个可恢复 full backup set 及其必需 WAL。
- 至少一个加密备份仓库位于 PostgreSQL 主机和 `postgres-data` 卷之外；备份密钥不得存放在仓库、镜像、日志或证据包中。
- 恢复范围包含 cluster 全局对象、`dcim` schema/业务/审计数据、`alembic_version`、TimescaleDB 扩展、hypertable、chunk、job、压缩和保留策略。Redis、MQTT 和外部附件不属于本 Story 的数据库恢复范围。
- 故障切换采用受控手工 promotion、稳定数据库端点切换和旧主 `pg_rewind` 或全量重建；不声明自动故障检测、自动 failover、多主或自动 failback。应用多实例支持仍由 D39-05/Story 39.10 决定。
- 每月执行隔离恢复演练，每季度执行主备/站点切换演练；PostgreSQL、TimescaleDB 或备份工具版本变化后追加演练。RTO 从故障注入或恢复决定的受信 UTC 时间点开始，到应用连续通过 readiness 和关键读写为止；RPO 同时记录缺失的已确认提交数量和最大时间差。
- Story 39.7 是正式告警、统一时间线和最终门禁证据的完成前置。Story 39.3 可先实施备份、WAL、恢复、fencing 和原始指标生产端，但在 39.7 完成前不得签署 Story gate `PASS`。
...
#### D39-03 已记录决策（2026-08-18）

唯一维护者 `proecheng` 记录以下可观测性与发布证据边界；不要求 Charlie、Dana 或其他 BMAD 虚拟角色审批：

- 预生产证据窗口为绑定同一 Git SHA、不可变前后端镜像摘要和环境指纹的连续 `>=72` 小时；窗口内必须有 `12` 次关键 E2E 首次运行通过，首末合格运行跨度至少 72 小时，相邻运行至少间隔 5 小时。重试、跳过、镜像/配置漂移或超过 5 分钟的监控缺口不能被计为合格运行。
- HTTP 错误率采用滚动 5 分钟窗口：`>0.5%` warning，`>1.0%` critical，连续三次评估 `<=0.5%` 后恢复。健康探针和指标抓取不进入 RED 业务流量；未处理请求异常必须计为 500 并产生日志。
- 进程和网关 CPU/内存、主机和网关磁盘在持续 5 分钟 `>=80%` 时 warning、`>=90%` 时 critical，连续三次 `<75%` 后恢复。每日备份年龄 `>=26h` warning、`>=36h` 或状态/完整性失败 critical，新的已验证成功备份且年龄 `<26h` 后恢复。
- 网关积压以启用对象计算：任一数据源连续失败 `>=3` 次 warning，达到自身 `retry_max_failures`、启用网关心跳超过 5 分钟或显式离线为 critical；连续三次不存在上述状态后恢复。数据库、已配置 Redis/MQTT、readiness 或备份状态失败均不得静默标记健康。
- 每条告警记录严重度、实测值、阈值、首次观测/触发时间、责任人 `proecheng` 和可执行 runbook。持续资源告警在条件保持 5 分钟后触发；备份、依赖、心跳和显式失败可立即触发。
- MTTR 从合格 critical 告警首次触发的受信 UTC 时间开始，到 readiness 与关键 E2E 同时连续三次通过为止；确认/分派时间不能替代告警时间，必须保留 UTC/monotonic 原始时间线和三次恢复确认。
- 上线后年度可用性 SLO 为 `>=99.5%`，责任人为 `proecheng`；365 天错误预算为 `43.8` 小时（`2,628` 分钟）。缺失遥测不计为正常时间，维护窗口只有在区间开始前批准并记录时才允许排除。
- 72 小时窗口仅报告观测可用性和临时错误预算消耗，所有证据必须声明 `annual_slo_proven: false`。告警未关闭、窗口无效或 SLO/阈值违约时，发布及依赖门禁保持阻断，直到确定性恢复并建立具名整改工作；不得把短期窗口外推为年度证明。
...
|----|----------------------|------------|----------|
| D39-01 | 数值 RTO、RPO、备份保留、恢复范围和故障切换范围（已记录：2026-08-14，见 26.4） | `proecheng`（唯一维护者） | Story 39.3 |
